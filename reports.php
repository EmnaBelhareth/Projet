		...
		<div class="col-md-12">
			...
			<?php
				...
				if(($sdate<$t) && ($edate<=$t) && ($edate >= $sdate))
				{
					...
					$query_student = "SELECT etudiant.sid, etudiant.nom  from etudiant INNER JOIN student_subject WHERE etudiant.sid = student_subject.sid AND student_subject.id  = {$selsub}  ORDER BY etudiant.sid";
					$stu=$pdo->query($query_student);
